...
namespace MWS.Pages
{
    class LoginUczestnik : Page
    {
        public ActiveLine login { get; set; }
        public ActiveLine password { get; set; }

        public LoginUczestnik(StaticLine note = null): base(note)
        {
            Contents.Add(new StaticLine("LOGOWANIE UCZESTNIKA"));
        ...
        public override void React(Line line)
        {
            switch(line.Index)
            {
                ...
                case 3:
                    ...
                    if (log is null)
                        DisplayAdapter.Display(new LoginUczestnik(new StaticLine("Niepoprawne dane logowania. Spróbuj ponownie", ConsoleColor.Red)));
                    else
                    {
                        DisplayAdapter.Display(new PanelUczestnika(log));
                    }
                    break;
                case 4:
                    DisplayAdapter.Display(new LoginRegister(new LoginUczestnik()));
                    break;
                case 5:
