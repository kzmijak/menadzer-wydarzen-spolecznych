...
namespace MWS.Procedures
{
    class PSetPlatnosc : DatabaseObjectProcedures
    {
        ...
        public DatabaseObject GetRecord(DatabaseObject dbobject)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(DbHelper.CnnVal("cnMWS")))
            {
                return connection.Query<DatabaseObject>("dbo.Platnosc_GetRecord @id", dbobject).ToList()[0];
            }
        }
        ...
        public DatabaseObject GetRecordById(int id)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(DbHelper.CnnVal("cnMWS")))
            {
                return connection.Query<DatabaseObject>("dbo.Platnosc_GetRecord @id", new { id }).ToList()[0];
            }
        }
        ...
        public void Insert(DatabaseObject dbobject)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(DbHelper.CnnVal("cnMWS")))
            {
                connection.Execute("dbo.Platnosc_Insert @idkarty, @idadresata, @kwota, @dzien, @godzina", dbobject);
            }
        }
